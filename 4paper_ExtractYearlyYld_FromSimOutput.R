...
ExtracYld_csv<-function(runingdir,cropmodel,crop,ssp,fer,startyear,endyear){
  setwd(runningdir)
  ...
  for(t in 1:length(type_n)){
    outfile<-paste("./output/",cropmodel,"_",crop,"_yield_",type_n[t],"Sow_",fer,".txt",sep="")
  ...
  infile<-paste("./simout/",cropmodel,"_",crop,"_result_20crv3_obs_",fer,".csv",sep="")
  df<-read.csv(infile,header=T)
  #df<-read.table(infile,sep="",header = T,na.strings ="", stringsAsFactors= F) #DSSAT
  for(rowi in 1:nrow(grid)){
    yld<-df[df$SIMUNIT==grid$SIMUNIT[rowi],3:38] #epic
    #yld<-df[df$SIMUNIT==grid$SIMUNIT[rowi],(3+12):(38+12)]%>%mutate_all(as.numeric) #dssat
    yld[is.na(yld)] <- 0
      ...
      for(n in 1:length(type_n)){
        if(((n==1)&(length(mon[!is.na(mon)]>0)))|(n>1)){  #Some rows have no reported sowing month
          ...
          if(nrow(out)>0){write.table(cbind(grid$SIMUNIT[rowi],out),
                      file=paste("./output/",cropmodel,"_",crop,"_yield_",type_n[n],"Sow_",fer,".txt",sep=""),
...
ExtracYld_csv(runingdir,cropmodel,crop,ssp,fer,startyear=1971,endyear=2021)  #yield is a one csv table
######################################################################################################################################