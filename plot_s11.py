...
def AggregatedYld(fer):
    """Aggregate simulated yields by sowing window, maturity class, and season."""
    sw = ["ReportedSow", "OptimumFixedSow"]
    area = pd.read_csv(INPUT_DIR / "Africa_SimGrid_Confirmed_5min_4calibration.csv")[
    ...
    for sow in sw:
        df0 = pd.read_csv(OUTPUT_DIR / ("EPIC_mz_yield_" + sow + "_" + fer + ".txt"), sep=r"\s+")
        for cul in [1, 2, 3]:
            for sea in [1, 2]:
                df = df0[(df0["matu"] == cul) & (df0["sea"] == sea)].fillna(0).reset_index()
                area1 = area[["SIMUNIT", "se1m", "A"]].dropna()
                df = df.merge(area1, how="left").fillna(0)
                yld.loc[len(yld) + 1,] = [sow, cul, sea, 0] + list(
                    (np.dot(df[["A"]].values.T, df[[f"yld_{yr}" for yr in range(1971, 2022)]].values) / df["A"].sum()).flatten()
                )
                df["G1count"] = [(df.loc[i, [f"yld_{yr}" for yr in range(1971, 2022)]] >= 1).sum() for i in df.index]
                df = df[df.G1count >= (51 * 0.8)]
                yld.loc[len(yld) + 1,] = [sow, cul, sea, 1] + list(
                    (np.dot(df[["A"]].values.T, df[[f"yld_{yr}" for yr in range(1971, 2022)]].values) / df["A"].sum()).flatten()
                )
    return yld


def Supplementary_Plot_S11(fer,season):
    ...
    fig.savefig(PLOTS_DIR / "plot_s11.png",format="png",dpi=300, bbox_inches='tight', pad_inches=0)
    fig.savefig(PLOTS_DIR / "plot_s11.pdf",format="pdf",dpi=300, bbox_inches='tight', pad_inches=0)

if __name__ == "__main__":
    Supplementary_Plot_S11("wofer_gridcalibratedWaHi", season=1)
