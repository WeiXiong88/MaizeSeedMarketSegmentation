        ...
        for cul in [1,2,3]: #three varieties
            for sea in [1,2]: #two seasons only the first season because the sowing month identifed by peaks is not feasible
                ...
                yld.loc[len(yld)+1,]=[sow,cul,sea,1]+list((np.dot(df[['A']].values.T,
                                                    df[["yld_"+str(yr) for yr in range(1971,2022)]].values)/df['A'].sum()).flatten())
    return(yld)
def Supplementary_Plot_S9(fer,season):
    ...
    fig.savefig(PLOTS_DIR / "plot_s9.png",format="png",dpi=300, bbox_inches='tight', pad_inches=0)
    fig.savefig(PLOTS_DIR / "plot_s9.pdf",format="pdf",dpi=300, bbox_inches='tight', pad_inches=0)

if __name__ == "__main__":
    Supplementary_Plot_S9("wfer_gridcalibratedWaHi", season=2)
