...
optimCultivar<-function(df){ #input: dataframe or matrix
  ...
  for(i in 1:3){
    ...
    se1m<-which.max(meanyld)
      ...
      mg[i,]<-c(mean(yld[,se1m])^2/(100*max(0.01*mean(yld[,se1m]),sd(yld[,se1m]))),se1m,mean(yld[,se1m]),sum(yld[,se1m]>=0.2)) #1season SI
  #}
  }
  ...
  if(!is.na(result[1]) & !is.na(result[2])){ #there is primary season
    ...
    for(i in 1:3){
      ...
      se2m<-which.max(meanyld)                 #2 season month
        ...
        mg[i,]<-c(mean(yld[,se2m])^2/(100*max(0.01*mean(yld[,se2m]),sd(yld[,se2m]))),se2m,mean(yld[,se2m]),sum(yld[,se2m]>=0.2))
      #}
    }
...
segmentation<-function(infile,outfile,crop,startyear,endyear){
  ...
  #df<-read.table(infile,sep="",header = T,na.strings ="", stringsAsFactors= F)%>%filter((year>=startyear)&(year<=endyear)) #DSSAT
  ...
  for(unit in unique(df$SIMUNIT)){
    #df0<-df[df['SIMUNIT']==unit,(3+12):(38+12)]%>%mutate_all(as.numeric)  #DSSAT
    df0<-df[df['SIMUNIT']==unit,3:38]  #EPIC
    df0[is.na(df0)] <- 0
...
G1<-"_GT0.2"
infile<-paste("D:/works/AfricaMzSg/simout/EPIC_mz_result_20crv3_obs_",fer,".csv",sep="")
for (i in 1:5){
...
#for CERES-Maize and CSM-IXIM result
crop<-"mz"
fer<-"wofer_gridcalibratedWaHi"
G1<-"_GT0.2"
infile<-paste("D:/works/AfricaMzSg/simout/EPIC_mz_result_20crv3_obs_",fer,".csv",sep="")
outfile<-paste("D:/works/AfricaMzSg/results/",crop,"/",fer,"/MZCER_",crop,"_culseg_",fer,"_1971-2021",G1,".csv",sep="")
segmentation(infile,outfile,crop,1971,2021)
